-- La versión coincide con el registro remoto de Supabase.
begin;

-- Conserva las asignaciones de productos existentes y adopta la URL plural usada por el mega menú.
...
-- Las categorías antiguas se mantienen sincronizadas con las raíces comerciales.
insert into public.categorias (nombre, slug, activa)
select source.nombre, source.slug, true
from (values
  ('Combos', 'combos'),
  ('Líneas y Aparejos', 'lineas-y-aparejos'),
...
-- Raíces comerciales que ya están representadas en el mega menú.
...
from (values
  ('Combos', 'combos', 1),
  ('Líneas y Aparejos', 'lineas-y-aparejos', 5),
...
-- Opciones de primer nivel: son las que aparecerán directamente al clasificar un producto.
insert into public.catalogo_nodos (parent_id, nombre, slug, nivel, descripcion, activo, orden)
select root.id, source.nombre, source.slug, 'Clasificación', '', true, source.orden
from (values
  ...
  ('senuelos', 'Accesorios para señuelos', 'accesorios-para-senuelos', 7),
  ...
  ('herramientas-y-accesorios', 'Grips / Básculas', 'grips-basculas', 2),
  ('herramientas-y-accesorios', 'Tijeras / Corta líneas', 'tijeras-corta-lineas', 3),
  ...
  ('indumentaria', 'Buff / Máscaras', 'buff-mascaras', 6),
...
-- Subniveles que el mega menú usa dentro de accesorios para señuelos.
insert into public.catalogo_nodos (parent_id, nombre, slug, nivel, descripcion, activo, orden)
select parent_node.id, source.nombre, source.slug, 'Subclasificación', '', true, source.orden
from (values
...
from (values
  ...
  ('senuelos', 'Accesorios para señuelos', 'accesorios-para-senuelos'),
  ...
  ('herramientas-y-accesorios', 'Grips / Básculas', 'grips-basculas'),
  ('herramientas-y-accesorios', 'Tijeras / Corta líneas', 'tijeras-corta-lineas'),
  ...
  ('indumentaria', 'Buff / Máscaras', 'buff-mascaras'),
...
-- Cada slug debe ser único dentro de su mismo padre para que una URL resuelva un solo nodo.
