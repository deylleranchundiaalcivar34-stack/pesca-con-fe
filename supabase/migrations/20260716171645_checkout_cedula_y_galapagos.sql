-- Valida una cédula ecuatoriana de persona natural (10 dígitos y módulo 10).
...
as $$
declare
  cedula text := regexp_replace(trim(cedula_input), '\\D', '', 'g');
  indice integer;
  digito integer;
  acumulado integer := 0;
  verificador integer;
...
revoke all on function private.es_cedula_ecuatoriana_valida(text) from public;

-- Solo el cliente autenticado puede registrar su propia cédula validada.
...
declare
  ...
  cedula_normalizada text := regexp_replace(trim(coalesce(cedula_input, '')), '\\D', '', 'g');
begin
...
revoke all on function public.registrar_cedula_checkout(text) from public;
grant execute on function public.registrar_cedula_checkout(text) to authenticated;

-- El pedido toma una copia de la cédula validada y mantiene Galápagos sin tarifa fija.
...
begin
  ...
  new.cliente_cedula := regexp_replace(cedula_cliente, '\\D', '', 'g');
  provincia_normalizada := translate(lower(trim(coalesce(new.cliente_provincia, ''))), 'áéíóúüñ', 'aeiouun');
...
drop trigger if exists preparar_datos_envio_pedido_insert on public.pedidos;
create trigger preparar_datos_envio_pedido_insert
before insert on public.pedidos
for each row execute function private.preparar_datos_envio_pedido();

drop trigger if exists preparar_datos_envio_pedido_update on public.pedidos;
create trigger preparar_datos_envio_pedido_update
before update of cliente_id, tipo_entrega, cliente_provincia, subtotal, envio, total on public.pedidos
for each row execute function private.preparar_datos_envio_pedido();

-- El total de Galápagos requiere cotización manual, por lo que no puede ir a PayPhone.
...
drop trigger if exists bloquear_payphone_galapagos on public.pedidos;
create trigger bloquear_payphone_galapagos
before update of metodo_pago, proveedor_pago on public.pedidos
for each row execute function private.bloquear_payphone_galapagos();

revoke all on function private.preparar_datos_envio_pedido() from public;
revoke all on function private.bloquear_payphone_galapagos() from public;
